...
export interface GameSettings {
  ...
  allowManualSelection: boolean;     // lets player pick a specific segment
  equalizeWeights: boolean;         // treats all segments as weight 1
}
...
export class SettingsService {
  ...
  toggleManualSelection(): void {
    const currentSettings = this.currentSettings;
    const newSettings = { ...currentSettings, allowManualSelection: !currentSettings.allowManualSelection };
    this.updateSettings(newSettings);
  }

  toggleEqualizeWeights(): void {
    const currentSettings = this.currentSettings;
    const newSettings = { ...currentSettings, equalizeWeights: !currentSettings.equalizeWeights };
    this.updateSettings(newSettings);
  }
