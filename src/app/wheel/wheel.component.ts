...
export class WheelComponent implements AfterViewInit, OnChanges {
  ...
  @Input() allowManualSelection = false; // when true the user can pick a segment before spinning
  @Input() equalizeWeights = false; // when true all segments are treated as weight 1 (more fair)
  ...
  private manualChoiceIndex: number | null = null;
  ...
  constructor(
    ...
    this.darkMode = this.darkModeService.darkMode$;    this.canvasHeight = Math.min(window.innerHeight, window.innerWidth) * 0.50;
  ...
  /**
   * Allows parent component to bypass RNG and specify which index should win.
   * When allowManualSelection is true a simple UI is rendered so the user can
   * call this method by tapping one of the buttons. It will trigger a normal
   * spin animation but the outcome is deterministic.
   */
  ...
  spinWheel(): void {
    ...
    // if a manual choice was made, override the random pick
    if (this.manualChoiceIndex !== null) {
      this.winningNumber = this.manualChoiceIndex;
      this.manualChoiceIndex = null;
    } else {
    ...
    let winningAngle = 0;
    const weightOfWinning = this.equalizeWeights ? 1 : this.items[this.winningNumber].weight;
    const winningSegmentSize = arcSize * weightOfWinning;

    for (let index = 0; index < this.items.length; index++) {
      const item = this.items[index];
      const w = this.equalizeWeights ? 1 : item.weight;
      winningAngle += arcSize * w;
  ...
  private getTotalWeights(): number {
    if (this.equalizeWeights) {
      return this.translatedItems.length;
    }
  ...
  getRandomWeightedIndex(): number {
    ...
    for (let i = 0; i < this.translatedItems.length; i++) {
      const w = this.equalizeWeights ? 1 : this.translatedItems[i].weight;
      accumulatedWeight += w;
